...
export async function POST(request: NextRequest) {
  try {
    ...
    // 转发请求到外部API
    const response = await fetch(`${AUTH_API_URL}/captcha/email/verify`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.message || '邮箱验证失败' }, { status: response.status });
    }
    ...
    console.error('Verify email captcha error:', error);
